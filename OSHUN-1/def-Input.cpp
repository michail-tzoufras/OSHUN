...
//      spherical harmonics (assumed l0 > 3, m0 > 1, l0 >= m0)
        l0 = 3;   m0 = 2; 
        nump = 108; numx_glob = 30; numy_glob = 2;

        xmin = -1243.7; xmax = 1243.7;
        ymin = -5000.0; ymax = 5000.0;
        pmax = 0.15; pmin = pmax /(2.0*nump);
...
//      Collisions for f00
        ...
        smaller_dt = 0.005;
        NB_algorithms = 4;
        if_tridiagonal = false;
        implicit_E     = false;

//      loop control 
        RKLevel = 2; 
        n_outsteps = 40; // every 250 fs
        t_stop =  3880.0; //  240 ps

//      Restat files
        restart_time = 0; // start reading at the "restart_time"-th restart file
        n_restarts = 1;   // generate "n_restarts" restart files
...
//      Density profile
        ...
        xloc.push_back(   -1243.7  );   xdens.push_back(   1.00  ); 
 ...
 //     Temperature profile
        ...
        setup_sine_temp_x = true;
        pt_sinmax = 0.025;

        // X
        setup_pcwise_tmp_x = false;
        xtloc.push_back(   -1243.7 );    xtemp.push_back(   0.037  ); // you can't set pth = 0.0  
        xtloc.push_back(   -425.0  );   xtemp.push_back(   0.037  ); 
        xtloc.push_back(   -100.0 );    xtemp.push_back(   0.04625  );
        xtloc.push_back(    100.0 );    xtemp.push_back(   0.04625  ); 
        xtloc.push_back(    425.0  );   xtemp.push_back(   0.037  ); 
        xtloc.push_back(    1243.7 );    xtemp.push_back(   0.037  );
        ...
        ytloc.push_back(  -5000.0 );   ytemp.push_back(   1.00  ); 
        ytloc.push_back(  -3.0  );   ytemp.push_back(   1.00  ); 
